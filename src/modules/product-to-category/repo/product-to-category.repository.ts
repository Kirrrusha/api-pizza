...
/*
    add method check count rellatives - compare - not update relatives when theren't changes
*/
...
export class ProductToCategoryRepository {
  ...
  async save(saveProductCategoryDto: SaveProductCategoryDto): Promise<ProductToCategory> {
    ...
    try {
      ...
      const result = await this.productToCategoryDBProvider
        .findOneAndUpdate({ productId, categoryId }, { productId, categoryId }, { new: true, upsert: true })
        .lean()
        .exec()
  ...
  async getAllById(id: Types.ObjectId, type: 'categoryId' | 'productId'): Promise<ProductToCategory[]> {
  ...
  async saveMany(payload: ProductToCategory[]): Promise<ProductToCategory[]> {
    const prefix = payload.reduce((acc, item) => `${acc} [Product IDs: ${item.productId}][Category ID: ${item.categoryId}]`, '')
  ...
  async remove(saveProductCategoryDto: SaveProductCategoryDto): Promise<void> {
    const { productId, categoryId } = saveProductCategoryDto
    const prefix = `${productId ? `[PRODUCT ID: ${productId}]` : ''} ${categoryId ? `[CATEGORY ID: ${categoryId}]` : ''}`
