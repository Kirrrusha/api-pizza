...
export class ProductToOptionRepository {
  ...
  async save(saveProductToOptionDto: SaveProductToOptionDto): Promise<ProductToOption> {
    ...
    try {
      ...
      const result = await this.productToOptionDBProvider
        .findOneAndUpdate({ productId, optionId }, { productId, optionId }, { new: true, upsert: true })
        .lean()
        .exec()
  ...
  async getAllById(id: Types.ObjectId, type: 'optionId' | 'productId'): Promise<ProductToOption[]> {
  ...
  async saveMany(payload: ProductToOption[]): Promise<ProductToOption[]> {
    const prefix = payload.reduce((acc, item) => `${acc} [Product IDs: ${item.productId}][OPTION ID: ${item.optionId}]`, '')
  ...
  async remove(saveProductToOptionDto: SaveProductToOptionDto): Promise<void> {
    ...
    const prefix = `${productId ? `[PRODUCT ID: ${productId}]` : ''} ${optionId ? `[CATEGORY ID: ${optionId}]` : ''}`
